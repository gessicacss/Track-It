...
async function handleClick(id, done) {
  ...
  return (
    <ContainerHabits data-test="today-habit-container">
      <LeftSide current={habit.highestSequence} highest={habit.highestSequence} done={habit.done}>
        <h4 data-test="today-habit-name">{habit.name}</h4>
        <p data-test="today-habit-sequence">
          Sequência atual: 
          <span> {habit.currentSequence} {habit.currentSequence > 1 ? 'dias' : 'dia'}
          </span>
        </p>
        <p data-test="today-habit-record">
          Seu recorde: 
          <span> {habit.highestSequence} {habit.highestSequence > 1 ? 'dias' : 'dia'}</span>
        </p>
      </LeftSide>
      <RightSide
